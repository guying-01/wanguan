...
Public.getDefaultPage = function(){
	...
	try{
		do{
			if (/home\/index/.test(win.location.href)) {
				return win;
			}
			win = win.parent;
			i--;
...
Business.categoryCombo = function($_obj, opts, type){
	...
	//新增分类
	var addb = '';
	if(typeNumber == 'CustomLevel')
		addb = $('#quickAddCustomLevel');
	else
		addb = $('#quickAddCategory');
	...
	addb.on('click', function(e){
		...
		if(typeNumber == 'CustomLevel')
			Public.customLevelPop(typeNumber,window.parent,callback);
		else
...
Public.zTree = {
    ...
    init: function($target, opts, setting ,callback) {
        ...
        var skey = "";
        if(opts.skey) skey = "&tree=1&&skey=" + opts.skey;
        Public.ajaxPost(opts.url || '../basedata/assist?action=list&typeNumber=trade&isDelete=2' + skey, {}, function(data) {
            if (data.status === 200 && data.data) {
...
Public.categoryTree = function($obj, opts) {
	...
	var readonly = opts.readonly ? 'readonly' : 'onkeyup="scategory(this)"';
	var _Combo = {
		...
		selectByValue:function(value){
			if(value !== ''){
				if(this.data){
					...
					if(this.data[value]) this.obj.val(this.data[value].name);
				}
			}
			return this;
		}
...
Public.customLevelPop = function(type,targetWin,callback){ 
	var defaultPage = Public.getDefaultPage();
	var self = {
			init:function(){
				var myParent = targetWin || parent;
				...
				var content = $(['<form id="manage-form" action="" style="width: 282px;">',
				               '<ul class="mod-form-rows manage-wrap" id="manager">',
						           '<li class="row-item">',
						               '<div class="label-wrap"><label for="category">等级:</label></div>',
						               '<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="customlevel" id="customlevel" style="width:190px;"></div>',
						           '</li>',
						       '</ul>',
					       '</form>'].join(''));
				var height = 90;
				var dialog = myParent.$.dialog({
					...
					okVal:'确定',
					ok:function(){
						var	category = $.trim(content.find('#customlevel').val());
						if(!category){
							defaultPage.Public.tips({content : '请输入等级名称！'});
							category.focus();
							return false;
						}
						var oper = 'add'; 
