...
function initEvent() {
	$("#btn-add").click(function(a) {
        ...
        a.preventDefault(), verifyRight(rightsAction.import) && parent.$.dialog({
            width: 560,
            height: 300,
            title: "批量导入",
            ...
            lock: !0,
            data: {
                category: 'category_' + conditions.typeNumber,
                callback: function() {
        ...
        if (verifyRight(rightsAction.export)) {
            var a = "输入名称或编码查询" === $("#matchCon").val() ? "" : $.trim($("#matchCon").val());
            $(this).attr("href", "../basedata/assist/exporter?action=exporter&isDelete=2&typeNumber=" + conditions.typeNumber + "&skey=" + a)
        }
		...
		conditions.skey = "输入名称或编码查询" == b ? "" : b, $("#grid").setGridParam({
			postData: conditions
		}).trigger("reloadGrid")
...
function postData(a) {
	...
	var f = '';
	if(conditions.name == '商品') {
		f = { parentId: e, id: a, name: b, code: $("#code").val(), remark: $("#remark").val(),};
	} else {
		f = { parentId: e, id: a, name: b };
	}

	var g = "add" == d ? "新增" + conditions.name + "类别" : "修改" + conditions.name + "类别";
...
function verifyRight(a) {
	...
	case rightsAction.update:
		break;
	case rightsAction.import:
		break;
	case rightsAction.export:
		break;
	default:
		return !1
	}
...
var conditions = {
	...
	handle = {
		operate: function(a, b) {
			...
			if(conditions.name == '商品')
			{
				d = ['<form id="manage-form" action="">', '<ul class="mod-form-rows manage-wrap" id="manager">', '<li class="row-item" style="position:relative; display:none;">', '<div class="label-wrap"><label for="ParentCategory">上级分类:</label></div>', '<div class="ctn-wrap" style="position:relative;"><input type="text" value="" class="ui-input" name="ParentCategory" id="ParentCategory" readonly></div>', '<div class="dn hideFeild"></div>', "</li>", '<li class="row-item">', '<div class="label-wrap"><label for="category">编码:</label></div>', '<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="code" id="code"></div>', "</li>", '<li class="row-item">', '<div class="label-wrap"><label for="category">类别:</label></div>', '<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="category" id="category"></div>', "</li>", '<li class="row-item">', '<div class="label-wrap"><label for="category">说明:</label></div>', '<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="remark" id="remark"></div>', "</li>", "</ul>", "</form>"];
			} else{
