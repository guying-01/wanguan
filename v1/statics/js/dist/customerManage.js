...
function getCustomerData() {
	var a = getEntriesData(),
		...
		c = {
			...
			cLevel: customerLevel.getValue(),
			cLevelName: customerLevel.getText(),
			beginDate: $("#date").val(),
...
function initField() {
	if ($("#note").placeholder(), "edit" == oper) {
		if ($("#number").val(rowData.number), $("#name").val(rowData.name), $("#category").data("defItem", ["id", rowData.cCategory]), $("#customerLevel").data("defItem", ["id", rowData.cLevel]), rowData.beginDate) {
...
function initEvent() {
	...
	var z = "CustomLevel";
	customerLevel = Business.categoryCombo($("#customerLevel"), {
		defaultSelected: $("#customerLevel").data("defItem") || void 0,
		extraListHtml: '<a href="javascript:void(0);" id="quickAddCustomLevel" class="quick-add-link"><i class="ui-icon-add"></i>新增等级</a>',
		editable: !0,
		trigger: !0,
		width: 210,
		ajaxOptions: {
			formatData: function(b) {
				...
				if (200 == b.status) {
					...
					return c.SYSTEM.categoryInfo = c.SYSTEM.categoryInfo || {}, c.SYSTEM.categoryInfo[z] = b.data.items, b.data.items, b.data.items
				}
				return []
			}
		}
		...
		}, 
		{
            ...
            formatter: function(cellvalue, options, rowObject) {
				if (cellvalue != undefined) {
                    var b = /^(1)[\d]{10}$/;
                    return  b.test(cellvalue) ? cellvalue : '';
				}
				return '';
			},
            editrules: {custom:true, custom_func:function(val, tt){
	                var b = /^(1)[\d]{10}$/;
	                return b.test(val) ? [true, ""] : [false, "请填写正确的手机号码"]
				}
			}
		},
		{
