function initEvent() {
	$("#btn-add").click(function(a) {
		...
		a.preventDefault(), $("#grid").jqGrid("setGridParam", {
			url: "../basedata/assist?action=list&typeNumber=CustomLevel&isDelete=2",
			datatype: "json"
		}).trigger("reloadGrid")
	...
	$("#grid").jqGrid({
		...
		loadComplete: function(a) {
			if (a && 200 == a.status) {
				...
				var e = 250 == a.status ? "没有客户等级数据！" : "获取客户等级数据失败！" + a.msg;
				parent.Public.tips({
					type: 2,
					content: e
				})
...
var handle = {
	operate: function(a, b) {
		...
		$.dialog({
			title: c,
			content: "url:custom_level_manage",
			data: d,
			width: 400,
			height: 100,
			max: !1,
	...
	del: function(a) {
		Business.verifyRight("CL_DELETE") && $.dialog.confirm("删除的客户等级将不能恢复，请确认是否删除？", function() {
			Public.ajaxPost("../basedata/assist/delete?action=delete&typeNumber=CustomLevel", {
				id: a
			}, function(b) {
