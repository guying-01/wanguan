...
class Contact extends CI_Controller {
			...
			if (strlen($row['linkMans'])>0) {                            
				...
				foreach ($list as $arr1=>$row1) {
					if ($row1['linkFirst']==1) {
						...
						$v[$arr]['address']			 = isset($row1['address']) ? $row1['address'] : ''; 
	...
	public function query() {    
		...
		if (count($data)>0) {
			...
			$info['name']         = $data['name'];
			$info['beginDate']    = $data['beginDate'] && $data['beginDate'] == date('Y-m-d', strtotime($data['beginDate'])) ? $data['beginDate'] : '';
	...
	public function add(){
		...
		$data = elements(array(
					...
					,'periodMoney','remark','type','difMoney','contact','pinYin')
					,$data);
		$data['beginDate'] = $data['beginDate'] ? $data['beginDate'] : null;
	...
	public function update(){
		...
		$info = elements(array(
					...
					,'periodMoney','remark','type','difMoney','contact','pinYin')
					,$data);
		$info['beginDate'] = $info['beginDate'] ? $info['beginDate'] : null;
	...
	private function validform($data) {
		...
		$data['beginDate']     = $data['beginDate'] && $data['beginDate'] == date('Y-m-d', strtotime($data['beginDate'])) ? $data['beginDate'] : '';
