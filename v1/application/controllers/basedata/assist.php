...
class Assist extends CI_Controller {
	...
	public function index(){
		...
		$tree   = str_enhtml($this->input->get('tree',TRUE));
		$where = ' and typenumber="'.$type.'"';
		$where .= $skey ? ' and (name like "%'.$skey.'%" or code like "%'.$skey.'%")' :'';
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['name']        = $row['name'];
			$v[$arr]['code']        = $row['code'];
			$v[$arr]['parentId']    = $tree ? '0' : intval($row['parentId']);
    ...
    private function trade_add($data){
		...
		if ($sql) {
			...
			die('{"status":200,"msg":"success","data":{"coId":0,"detail":true,"id":'.$newid.',"level":'.$data['level'].',"name":"'.$data['name'].'","parentId":'.intval($data['parentId']).',"remark":"","sortIndex":1,"status":0,"typeNumber":"'.$data['typeNumber'].'","uuid":""}}');
		}
		str_alert(-1,'添加失败');
	}
...
//		$category = $this->input->get('typeNumber', TRUE);
		$category = 'trade';
		...
		$sheet->setCellValue('A1', '商品类别编码')
			->setCellValue('B1', '商品类别名称')
			->setCellValue('C1', '上级分类编码')
			->setCellValue('D1', '说明');

		$where = "AND (c1.typeNumber='{$category}')";
		$skey = str_enhtml($this->input->get('skey',TRUE));
		$where .= $skey ? "AND (c1.name like '%{$skey}%' OR c1.code like '%{$skey}%')" : '';
		$sql = "SELECT 
  c1.name,
  c1.code,
  c2.code parentCode,
...
ORDER BY c1.path";
		...
		$categories = $this->mysql_model->query(CATEGORY, $sql, 2);

		$count = count($categories);
		for ($i = 0; $i < $count; $i++) {
			$sheet->setCellValue('A' . ($i + 2), ' ' .$categories[$i]['code']);
			$sheet->setCellValue('B' . ($i + 2), $categories[$i]['name']);
			$sheet->setCellValue('C' . ($i + 2), ' ' . $categories[$i]['parentCode']);
			$sheet->setCellValue('D' . ($i + 2), $categories[$i]['remark']);
		}

		$name = 'category_' . $category . '_' . date('YmdHis') . '.xls';
		sys_csv($name);
		$this->common_model->logs('导出:'.$name);
